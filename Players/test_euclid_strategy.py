...
class TestEuclidStrategy(unittest.TestCase):
    """Tests for the `EuclidStrategy` class."""

    def setUp(self):
        self.strategy = EuclidStrategy()
    ...
    def test_get_action_can_reach_goal(self):
        self.assertEqual(
            self.strategy.get_action(self.p1_state),
            TurnWithMove(0, ShiftOp(Coord(4, 4), Direction.UP), Coord(3, 3)),
        )

    def test_get_action_cannot_reach_goal(self):
        self.assertEqual(
            self.strategy.get_action(self.p2_state),
            # rotate line tile to vertical
            #                      ↓ start
            # top rows become: ┐│┌┐│
            #                  └┘│└┘
            #                  ──└──
            #                     ↑ end (one tile above goal)
            TurnWithMove(270, ShiftOp(Coord(4, 0), Direction.LEFT), Coord(3, 2)),
        )
    ...
    def test_get_action_cannot_reach_home_goal(self):
        ...
        self.assertEqual(
            ...
            # bottom rows become: ┌└│┌┐
            #                     ┘│└┤─  ← inserted
            TurnWithMove(0, ShiftOp(Coord(4, 4), Direction.LEFT), Coord(2, 3)),
        )
